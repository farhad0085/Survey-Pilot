...
class TimezoneOffsetMiddleware(MiddlewareMixin):
    def process_request(self, request):
        try:
            offset = int(request.headers.get('X-Api-Timezone-Offset'))
        except:
            offset = 0
        # set this offset to request object
        setattr(request, 'tzoffset', offset)
